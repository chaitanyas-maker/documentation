...
title: Native App Automation with MCP
hide_title: false
...
site_name: TestMu AI
slug: native-app-automation-mcp/
canonical: https://www.testmuai.com/support/docs/native-app-automation-mcp/
---
      ...
      dangerouslySetInnerHTML={{ __html: JSON.stringify({
       "@context": "https://schema.org",
        ...
        "itemListElement": [{
          ...
          "@type": "ListItem",
          "position": 3,
          "name": "Native App Automation with MCP",
          "item": `${BRAND_URL}/support/docs/native-app-automation-mcp/`
        }]
      })
    }}
></script>
...
If your client does not prompt you, state the test type explicitly in your prompt, for example: "This is a mobile app automation test. Pull the failure reason for TestID `<testID>`." This avoids the tool defaulting to the web endpoint, which returns a **404 (Unable to find test details)** for a mobile TestID. See [Troubleshooting](#404-unable-to-find-test-details) if you hit this.

### Example prompts

- "This is a mobile app automation test. What is the failure reason for TestID `<testID>`?"
- "Pull the command logs for app-automation TestID `<testID>` and tell me which step failed."
- "Analyze the result for TestID `<testID>` and highlight the root cause."
...
- **Ask for the failure reason, not the full log.** Prompt specifically: "Highlight the failure reason for TestID `<testID>`," rather than "analyze all logs." This lets the tool return the relevant slice instead of the entire history.
