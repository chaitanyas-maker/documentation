...
Layout Regions persist across builds like any other annotated region. They are supported on CLI builds generated with the `exec` command, on Webhooks builds, and on PDF comparisons. See [Layout Regions](/support/docs/smartui-layout-regions/) for the full workflow.
